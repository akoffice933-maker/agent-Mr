  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        url: { type: "string", description: "URL сайта для объявления (Яндекс Директ: необязательно, но рекомендуется)" },
        ad_group_name: { type: "string", description: "Название группы объявлений" },
        title: { type: "string", description: "Основной заголовок объявления (используется, если не задан titles)" },
        text: { type: "string", description: "Текст объявления (описание, до 81 символа)" },
        titles: { type: "array", items: { type: "string" }, description: "Заголовки комбинаторного объявления Яндекса: до 7 штук, по 56 символов. Первый — основной." },
        callouts: { type: "array", items: { type: "string" }, description: "Уточнения (краткие фразы-преимущества) Яндекса: до 5, по 25 символов" },
        price_rubles: { type: "number", description: "Цена товара/услуги в объявлении, ₽" },
        price_old_rubles: { type: "number", description: "Старая (зачёркнутая) цена, ₽" },
        price_qualifier: { type: "string", enum: ["from", "up_to"], description: "Квалификатор цены: from — «от», up_to — «до»" },
        tracking_params: { type: "string", description: "Параметры URL / UTM-теги, добавляются ко всем ссылкам кампании (например utm_source=agentmr&utm_medium=cpc). Не задавать, если пользователь не просил." },
        images: { type: "array", items: { type: "object", properties: { url: { type: "string" }, name: { type: "string" } }, required: ["url"], additionalProperties: false }, description: "Изображения объявления (публичные URL, jpg/png/gif, до 5). Не задавать, если пользователь не дал ссылки." },
        keywords: { type: "array", items: { type: "string" }, description: "Ключевые фразы" },
        negative_keywords: { type: "array", items: { type: "string" }, description: "Минус-фразы" },
        region_ids: { type: "array", items: { type: "number" }, description: "ID регионов Яндекс; по умолчанию вся Россия" },
      },
      additionalProperties: false,
    },
  },
