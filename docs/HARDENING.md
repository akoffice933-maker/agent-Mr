...
> **Регрессия найдена и исправлена (внешнее pre-production ревью, 24.08.2026):**
> squash миграций в `0000_exotic_the_enforcers.sql` унёс с собой рассинхрон
> между `docs/rls_multi_tenant.sql` и `scripts/rls-audit.ts`: `metrics_daily`,
> `keywords`, `negative_keywords`, `avito_chats` имели `CREATE POLICY`, но RLS
> на них не был `ENABLE`/`FORCE` — политики были инертны, эти 4 таблицы
> фактически не изолировались между организациями. Отдельно `organizations`/
> `org_members`/`api_keys` ошибочно получали RLS, хотя design (identity-плоскость,
> см. п.2 выше) требует их исключения; `oauth_states` отсутствовал в списках
> `rls-audit.ts` и считался «unknown table». Всё исправлено, `rls-audit.ts`
> зелёный на живой проверке, регрессия закрыта CI-шагом ниже (Postgres-сервис
> в `ci.yml`, которого раньше не было — все DB-тесты раньше молча скипались).
...
> **Регрессия найдена и исправлена (внешнее pre-production ревью, 24.08.2026):**
> `DirectApi.call()` определял наличие API-уровневой ошибки условием
> `raw.errors?.length && !raw.result` — но задокументированный контракт Директа
> (`result` и `errors` могут прийти в одном ответе одновременно, см. комментарий
> в `api.ts`) означал, что транзиентная ошибка с непустым `result` (например
> `{ result: { SuspendResults: [] }, errors: [{ Code: 13, ... }] }`) молча
> принималась за успех — retry не срабатывал, а фактическое изменение на
> площадке не происходило. Живой прогон `tests/execution-pipeline.test.ts`
> подтвердил: E7 (retry на транзиентной ошибке) падал. Условие исправлено на
> `raw.errors?.length` без привязки к `result`. Заодно исправлена сама утечка
> состояния в тестовом файле: `const sim = createSimulator({ campaigns:
> [...simCampaigns] })` копировал только массив, а не объекты кампаний внутри —
> симулятор одного теста мутировал общие объекты, и это протекало в
> «свежие» симуляторы последующих тестов (замаскировано до фикса api.ts,
